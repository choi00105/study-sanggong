...
window.onload = () => {
	...
	//imageZone 영역 클릭 시 파일 이벤트 처리, 자바스크립트에선 이벤트 발생 시 이벤트 정보를 담고 있는 event(=e)라는 객체를 자동생성
	imgZone.addEventListener('click', (e) => {
		fileEvent.click(e) // 사용자가 만든 UI를 통해 <input type="file"> 태그를 클릭 한것과 같은 효과를 냄
		
	});

	// 파일 선택 창 열기 이벤트
	fileEvent.addEventListener('change', (e) => { // 파일 선택 해서 파일을 가져오는 거 
		const files = e.target.files; // 선택한 파일 정보가 e.target.files(<- 배열 형태)에 저장되어있음
		showImage(files); // 읽어온 파일을 인자로 받아서 이미지 미리보기를 실행
	});
	...
	const showImage = (files) => {
		...
		const reader = new FileReader(); // new 연산자를 통해서 FileReader()라는 객체를 reader가 참조(상속)
		reader.onload = function(event){ // 아래 img 태그로 만들어진 element에 이미지를 보낼 거라는걸 결정 -> 파일을 읽고나서 해야 할 일...
	...
	btnRegister.addEventListener('click', async () => {
		...
		if(gender.value === '') { alert("성별을 입력하세요."); return false; }
		if(hobby.value === '') { alert("취미를 입력하세요."); return false; }
		if(job.value === '') { alert("직업을 입력하세요."); return false; }
		if(description.value === '') { alert("자기 소개를 입력하세요."); description.focus(); return false; }
		...
		address.value = addr1.value + " " + addr2.value;
	 	...
	 	const telnoRegex = /^[\d\s-]+$/; // 숫자, -, 스페이스 만
	 	if(!telnoRegex.test(telno.value)) { alert('올바르지 않은 전화번호 형식 입니다.'); telno.focus(); return false;}
	 	//전화번호 문자열 정리
		...
		if(!emailRegex.test(email.value)) { alert('올바르지 않은 이메일 주소 형식 입니다.'); email.focus(); return false;}
		...
		*/
		...
		await fetch('/user/signup', { // ('경로', 보낼 데이터)
			method: 'POST',
			body: formData
