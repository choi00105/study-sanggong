...
const Login = () =>{
    ...
    const emailSaveRef = useRef();
    const passwordRef = useRef(); 
    const pwSaveRef = useRef();;
    ...
    const checBoxConfirm = async () => {
        ...
        if(emailCookie !== undefined && emaliSaveCookie !== undefined){ //email 쿠키가 존재하면 
        ...
        if(passwordCookie !== undefined && pwSaveCookie !== undefined){ //패스워드 쿠키가 존재하면
    ...
    const cookieManage = (username,role,authkey) => { 
        ...
        if(emailSaveRef.current === 'Y') 
                document.cookie = 'emailSave=' + emailSaveRef.current + ';path=/; expires=Sun, 31 Dec 2023 23:59:59 GMT';        
        ...
        if(pwSaveRef.current === 'Y') 
                document.cookie = 'pwSave=' + pwSaveRef.current + ';path=/; expires=Sun, 31 Dec 2023 23:59:59 GMT';        
    ...
    const loginCheck = async () =>{
        ...
        if(jwtRef.current.checked){
            await fetch('http://localhost:8080/restapi/loginCheck?autoLogin=JWT',{
                method : 'POST',
                body : formData
